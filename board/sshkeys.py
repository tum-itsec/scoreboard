...
def get_user_identifier(user_id, team_id, role):
    sep = current_app.config["SSH_ACCOUNT_NAME_SEPARATOR"]
    if current_app.config["SSH_TUTOR_ACCOUNTS"] and role in TUTOR_ROLES:
        return f"tut{sep}{user_id}"
    if team_id is not None:
        return f"team{sep}{team_id}"
    if current_app.config["SSH_STUDENT_ACCOUNTS"]:
        return f"stud{sep}{user_id}"
    return None
...
def sshkeys():
    ...
    cmd = current_app.config["SSHKEY_CMD"].format(user=user_identifier) if user_identifier else "Only team SSH accounts are available; form a team first"
...
def getkeys():
    ...
    for r in cur.fetchall():
        user_identifier = get_user_identifier(r["user_id"], r["team_id"], r["role"])
        if user_identifier:
